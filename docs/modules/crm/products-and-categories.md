...
1. На вкладке «Основное» задайте общее название и категорию и добавьте вариативные атрибуты.
2. Создайте варианты и заполните все значения вариативных атрибутов.
3. Для каждого варианта задайте цену, артикул, описание и изображения.
...
Вес и габариты задаются в карточке «Вариант товара», блок «Общая информация»: необязательные поля «Вес, г», «Длина, мм», «Ширина, мм» и «Высота, мм». В карточке самого товара этих полей нет.

- У вариативного товара карточку варианта открывают из списка вариантов на вкладке «Основное».
- У простого товара на экране «Каталог → Товары» переключите режим на «Отображать товары плоским списком», выделите строку и нажмите кнопку редактирования — откроется та же карточка «Вариант товара». Клик по названию открывает только просмотр, без полей веса.
- При [импорте](catalog-import.md) вес и габариты подтягиваются из YML-фида.

От веса зависит цена доставки перевозчиком (СДЭК, Почта России): вес посылки — сумма весов товаров заказа. Если вес не указан или равен нулю, вместо него берётся «Вес товара по умолчанию, г» из настройки перевозчика — см. [«Перевозчики: СДЭК и Почта России»](delivery-carriers.md#integration-tab). Габариты товара в расчёт пока не идут: размер посылки берётся из полей коробки в той же настройке — см. [«Как считается посылка»](delivery-carriers.md#parcel).
